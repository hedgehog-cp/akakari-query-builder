    ...
    <>
      ...
      {open && (
        <>
          ...
          <div
            class="fixed left-0 top-0 bottom-0 z-50 w-80 max-w-[90vw] bg-bg-panel border-r border-gray-300 shadow-lg p-3 flex flex-col"
          ...
          >
            ...
            <div class="flex gap-1 mb-3 text-xs">
              ...
              <button
                type="button"
                class="border border-emp-1 rounded px-2 py-0.5 hover:bg-emp-4 disabled:opacity-40"
                disabled={name.trim() === ""}
                onClick={save}
              >
                保存
            ...
            <ul class="flex-1 overflow-auto text-xs space-y-0.5">
              {templates.map((t) => (
                <li
                  key={t.name}
                  class="group flex items-center gap-1 px-1 py-1 rounded hover:bg-emp-4"
                >
                  {renaming === t.name ? (
                    <input
                      ref={renameInputRef}
                  ...
                  <span class="hidden group-hover:flex gap-1 shrink-0">
                    <button
                      type="button"
                      class="text-gray-500 hover:text-gray-800 px-1"
                      onClick={() => {
                        setRenaming(t.name);
              ...
              {templates.length === 0 && (
                <li class="text-gray-500 px-1 py-1">テンプレートはまだありません</li>
              )}
            </ul>
          </div>
        </>
