...
一致した行は先頭1万行までをそのまま繰って読めます。CSV/TSVでのコピーとCSVでのダウンロードは全件が対象です。
行はクリックで選べます(`Ctrl`で足し引き、`Shift`で範囲)。選んだ行は `Ctrl+C` でタブ区切りのまま写せます。
...
左端の 📁 から、いま作っている条件に名前を付けて残せます。`Ctrl+S` で引き出しが開き、名前を入れて `Enter` で保存します。
残した条件はブラウザの中に置かれ、一覧から呼び戻せます。
